...
Un concepto fundamental en evaluación de clasificadores es la **asimetría en el costo de los errores**: no todos los tipos de error tienen la misma gravedad para el negocio o el usuario. En un detector de spam, cometemos dos tipos de errores posibles: un **Falso Positivo (FP)** ocurre cuando un correo legítimo es marcado incorrectamente como spam, pudiendo hacer que el usuario pierda un mensaje importante —como una confirmación de vuelo o una oferta de trabajo—, lo que constituye un error **muy grave**. Un **Falso Negativo (FN)** ocurre cuando un mensaje de spam no es detectado y llega al inbox, siendo simplemente molesto pero sin causar daño real: un error **tolerable**.
